...
#define FREE(pointer) log_debug("%p being freed!",pointer);free(pointer)
...
void FillRequest(m4u_interfaces::Request* request)
{
   std::string key;
   std::string value;
...
void ListRequests(const m4u_interfaces::Request& request)
{
   log_debug("###########################################");
...
void new_answer_request(uv_stream_t *q, ssize_t nread, const uv_buf_t *buf)
{
   uv_shutdown_t* sreq;
   jint square;
   std::string output;
...
int main(int argc, char**argv) 
{
   ...
   if(argc>1)
   {
      ...
      log_debug("class path default[%s] %d:%s",default_class_path,argc,argv[0]);
