...
void new_answer_request(uv_stream_t *client, ssize_t nread, const uv_buf_t *buf)
{
   ...
   if(!ret)
   {
      ...
      //sprintf(body,"worker-Failed to parse request(%s) (%s).",buf->base,req_msg.DebugString().c_str());
      log_error("worker-Failed to parse request(%s) (%s).",buf->base,req_msg.DebugString().c_str());
      strcpy(body,r400);
   ...
   sprintf(page2,page,request.request_path().c_str(),request.name().c_str(),getpid(),num_request,request.body().c_str());
