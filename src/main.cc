...
static void
get_process_parent_id(const pid_t pid, pid_t* ppid)
{
  char* ptr;
  char buffer[BUFSIZ];
...
close_process_handle(uv_process_t* req, int64_t exit_status, int term_signal)
{
    ...
    if (found) {
      ...
      for (i = 0; i < p_service->instances; i++) {
        ...
        if (worker->req.pid != req->pid) {
          log_debug(
            "send kill to all child processes if they are my children!");
          get_process_parent_id(worker->req.pid, &ppid);
          // if the parent process is the server, then kill this client
          log_debug("ppid[%d] my pid[%d]", ppid, getpid());
          if (ppid == getpid()) {
  ...
  log_debug("Process[%d] exited with status %" PRId64 ", signal %d",
            req->pid,
            exit_status,
            term_signal);
...
static void
buf_alloc(uv_handle_t* handle, size_t suggested_size, uv_buf_t* buf)
{
  buf->base = (char*)calloc(1, suggested_size);
  buf->len = suggested_size;
}
void
after_read_pipe(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf)
{
  if (nread < 0) {
    if (nread != UV_EOF)
...
setup_workers(Service* service)
{
  ...
  while (cpu_count--) {
    ...
    ret =
      uv_read_start((uv_stream_t*)&worker->pipe, buf_alloc, after_read_pipe);
  }
}
//#############################################################################
...
void
int_handler(uv_signal_t* handle, int signum)
{
...
void
usr1_handler(uv_signal_t* handle, int signum)
{
  log_debug("SIGUSR1 received: %d", signum);
}

void
usr2_handler(uv_signal_t* handle, int signum)
{
  log_debug("SIGUSR2 received: %d", signum);
}

void
uv_on_exit(uv_process_t* req, int64_t exit_status, int term_signal)
{
...
static void
check_sockname(struct sockaddr* addr,
               ...
               const char* context)
{
  ...
  // ASSERT(memcmp(&check_addr.sin_addr, &compare_addr.sin_addr, sizeof
  // compare_addr.sin_addr) == 0);
...
void
after_read_admin(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf)
{
  uv_shutdown_t* sreq;
...
void
after_write_admin(uv_write_t* req, int status)
{
  write_req_t* wreq = (write_req_t*)req;
  if (status) {
    log_error("Write error %s", uv_err_name(status));
...
void
after_read(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf)
{
  write_req_t* req;
  uv_shutdown_t* sreq;
  int size;
  ...
  ret = scanner_plugin.p_scan(
    scanner, service_name, buf->base, strlen(buf->base), &path, &output, 0);
  ...
  if (path) {
    ...
    for (itor = services.begin(); itor != services.end(); itor++) {
      log_debug(
        "%d service[%s] try match[%s]", i++, itor->first.c_str(), match);
      p_service = itor->second;
    ...
    if (!found) {
      log_debug("Failed to match '%s' with '%s',returning %d.",
                match,
      ...
      /*req->buf.base = strdup((char*)r404); //= uv_buf_init((char*)r404,
      strlen(r404)+1); req->buf.len  = strlen(r404)+1;*/
    ...
    if (p_service && p_service->workers) {
      ...
      uv_write2((uv_write_t*)req,
                (uv_stream_t*)&worker->pipe,
                &req->buf,
                1,
                (uv_stream_t*)client,
                after_write);
      p_service->round_robin_counter =
        (p_service->round_robin_counter + 1) % p_service->instances;

      // fd is in the client space, close here to avoid problems with listen
      // maxfd backlog!
      close(fd);
      ...
      if (request.SerializeToString(&str_output)) {
        ...
        /*req->buf.base =
        strdup((char*)str_output.c_str());//uv_buf_init((char*)str_output.c_str(),
        str_output.length()+1); req->buf.len  = str_output.length()+1;*/
        req->buf =
          uv_buf_init((char*)str_output.c_str(), str_output.length() + 1);
      ...
      /*#else
                  ...
                  uv_write((uv_write_t*)write_req, (uv_stream_t*) &worker->pipe,
      &dummy_buf, 1, after_write);
...
void
on_connection_admin(uv_stream_t* server, int status)
{
  int ret;
  struct sockaddr sockname, peername;
  int namelen = sizeof peername;
  ...
  if (uv_accept(server, (uv_stream_t*)client) == 0) {
    ...
    uv_write(
      (uv_write_t*)req, (uv_stream_t*)client, &req->buf, 1, after_write_admin);
  } else {
    uv_shutdown_t* sreq;
    sreq = (uv_shutdown_t*)malloc(sizeof *sreq);
...
static int
CreateTcp4AdminServer(lua_State* L)
{
  int port = luaL_checkint(L, 1);
       ...
       const char* path)
{
  ...
  if (ret) {
    log_error("error compiling expression[%s](%d:%s)",
              expression,
              ret,
...
static int
RemoveBusinessObject(lua_State* L)
{
  Service* p_service;
  std::map<std::string, Service*>::iterator itor;
  const char* name = luaL_checkstring(L, 1);
...
static int
ListBusinessObjects(lua_State* L)
{
  std::map<std::string, Service*>::iterator itor;
  Service* p_service;
  std::stringstream ss;
...
int
load_plugin_library(const char* library_name,
                    const char* load_function,
                    const char* load_dummy_function)
{
...
RestScanner(lua_State* L)
{
  ...
  ret = scanner_plugin.p_scan(
    scanner, service_name, str, strlen(str), &path, &output, 0);
...
static int
SetJavaClassPath(lua_State* L)
{
  const char* str = luaL_checkstring(L, 1);
  log_debug("%s", str);
  if (java_class_path)
    free(java_class_path);
...
RunConfiguration(const char* filename)
{
  ...
  if (!java_class_path)
    java_class_path = strdup("-Djava.class.path=.");
