...
*/
const char*str_http_url_field(uint16_t field)
{
   static char ret[256];
   ret[0]='\0';
   if(field & UF_SCHEMA) strcat((char*)&ret,"schema|");
   if(field & UF_HOST) strcat((char*)&ret,"host|");
   if(field & UF_PORT) strcat((char*)&ret,"port|");
   if(field & UF_PATH) strcat((char*)&ret,"path|");
   if(field & UF_QUERY) strcat((char*)&ret,"query|");
   if(field & UF_FRAGMENT) strcat((char*)&ret,"fragment|");
   if(field & UF_USERINFO) strcat((char*)&ret,"user info|");
   if(field & UF_MAX) strcat((char*)&ret,"max|");
   return (const char*)&ret;
}
void print_url(const char *url,struct http_parser_url *u)
{
  unsigned int i;
  /*printf("\tfield_set: 0x%x, port: %u\n", u->field_set, u->port);
  printf("\tfield_set: 0x%x, port: %u\n", u->field_set, u->port);
...
main (void)
{
  ...
  struct http_parser_url u;
   memset(&u,0,sizeof(struct http_parser_url));
    http_parser_parse_url("abc://username:password@example.com:123/path/data?key=value#fragid1",
                               strlen("abc://username:password@example.com:123/path/data?key=value#fragid1"),
                               0,
                               &u);
    print_url("abc://username:password@example.com:123/path/data?key=value#fragid1",&u);
