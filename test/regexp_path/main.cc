...
AddBusinessObject("Servico SMS",BO_JAVA,"/SMS/[0-9]* /[0-9]*",5,"BO/JAVA/Sample.class")
*/
/*
...
const char * str_regcomp_error(int vlr)
{
   switch(vlr)
   {
...
int main()
{
   ...
   if(ret)
   {
      printf("error compiling expression[%s](%d:%s)",expression,ret,str_regcomp_error(ret));
   }
   ...
   for(itor=services.begin();itor!=services.end();itor++)
   {
      ...
      if (0 != (ret = regexec(&p_service->preg,match, nmatch , pmatch, 0))) {
         printf("Failed to match '%s' with '%s',returning %d.\n", match, p_service->regexp.c_str(), ret);
      } else {
